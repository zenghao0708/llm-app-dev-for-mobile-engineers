...
很多团队不是一开始就做跨平台端侧 AI，而是在 iOS 或 Android 上先验证能力，再迁移到另一个平台。迁移时不要直接复制代码，而要先抽取稳定契约：

1. 抽取任务输入输出 schema。
2. 抽取错误枚举和降级路径。
3. 抽取能力查询接口。
4. 抽取共享评测样本。
5. 保留平台 runtime 和权限实现。
6. 为新平台补齐性能基线。

迁移的目标不是两端代码完全一样，而是业务语义一致、风险边界一致、发布策略一致。例如 iOS 已经支持本地 OCR，Android 初期可以只支持云端兜底，但两端都应该返回同样的能力状态和错误模型。这样产品层可以统一展示“本地可用”“需要下载”“当前设备不支持”等状态。

迁移过程中最容易漏掉的是隐私和日志。一个平台可能已经做了脱敏预览，另一个平台只做了 UI 遮挡；一个平台日志只记录输入大小区间，另一个平台可能记录了完整文件路径。这些差异不能靠接口类型发现，必须通过隐私审查和共享测试用例发现。

## 13.17 跨平台团队协作方式

跨平台端侧 AI 涉及多个角色：共享层开发、iOS、Android、算法、服务端、测试、产品和安全。建议用三份文档降低沟通成本：

| 文档 | 内容 |
| --- | --- |
| 能力契约 | 支持能力、输入输出 schema、错误枚举、降级路径 |
| 平台差异表 | iOS/Android runtime、权限、性能、模型版本差异 |
| 发布矩阵 | 哪个平台、哪个版本、哪些设备、开放哪些能力 |

每次模型或能力更新，都要同时更新这三份文档。否则共享层以为能力已上线，某个平台却只是云端兜底；产品以为双端一致，测试却发现一个平台没有权限撤销路径。

跨平台协作还需要明确责任边界。共享层负责人负责状态机和契约稳定；平台负责人负责 runtime、权限、文件和性能；算法负责人负责模型版本和回归集；服务端负责人负责云端网关和配置；测试负责人负责双平台矩阵。责任不清时，端侧 AI 问题很容易在多个团队之间来回转移。

## 13.18 跨平台端侧 AI 的演进路线

跨平台能力可以按四阶段演进：

| 阶段 | 目标 |
| --- | --- |
| 第一阶段 | 建立能力查询、错误模型和降级框架 |
| 第二阶段 | 单平台开放本地能力，另一平台使用云端或传统兜底 |
| 第三阶段 | 双平台共享评测样本，完成主力设备矩阵 |
| 第四阶段 | 双平台独立灰度、独立回滚，但体验语义一致 |

这条路线允许团队先交付价值，再逐步补齐一致性。不要因为跨平台一致而拖慢所有平台，也不要因为某个平台领先而忽略另一个平台的降级体验。成熟跨平台工程的标志，是差异被清楚表达、被测试覆盖、被配置控制，而不是被隐藏。
